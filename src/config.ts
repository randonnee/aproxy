...
/** Keys written by older versions that are no longer honoured. */
const OBSOLETE_KEYS = ["proxyBackend"];
...
export function loadConfig(): AproxyConfig {
  try {
    ...
    const next = { ...defaults, ...parsed } as AproxyConfig & Record<string, unknown>;
